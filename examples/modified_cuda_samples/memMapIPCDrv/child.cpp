...
cuda::launch_configuration_t make_launch_config(const cuda::device_t &device, const cuda::kernel_t &kernel)
{
	...
	auto num_blocks_per_sm = cuda::kernel::occupancy::max_blocks_per_multiprocessor(kernel, num_threads_per_block, 0);
	auto num_blocks = num_blocks_per_sm * device.multiprocessor_count();
...
void childProcess(int devId, int id_of_this_child, char **)
{
	...
	auto module = cuda::module::create(device, fatbin.data());
